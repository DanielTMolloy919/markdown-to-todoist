...
import { TaskLayoutComponent, TaskLayoutOptions } from "./TaskLayoutOptions";
import { Recurrence } from "./Recurrence";
import { Task } from "./Task";
import { Priority } from "./Priority";
import { TaskRegularExpressions } from "./TaskRegularExpressions";
import type { TaskDetails, TaskSerializer } from "./taskIndex";
...
function symbolAndDateValue(
  ...
  return shortMode
    ? " " + symbol
    : ` ${symbol} ${date.format(TaskRegularExpressions.dateFormat)}`;
}
...
export class DefaultTaskSerializer implements TaskSerializer {
  ...
  public componentToString(
    ...
    switch (component) {
      ...
      default:
        throw new Error(
          `Don't know how to render task component of type '${component}'`,
        );
    }
  }
  ...
  public deserialize(line: string): TaskDetails {
    ...
    do {
      ...
      if (doneDateMatch !== null) {
        doneDate = moment(doneDateMatch[1], TaskRegularExpressions.dateFormat);
        line = line
      ...
      if (dueDateMatch !== null) {
        dueDate = moment(dueDateMatch[1], TaskRegularExpressions.dateFormat);
        line = line
      ...
      const dependsOnMatch = line.match(
        TaskFormatRegularExpressions.dependsOnRegex,
      );

      if (dependsOnMatch != null) {
        line = line
          .replace(TaskFormatRegularExpressions.dependsOnRegex, "")
          .trim();
