import { htmlEncodeCharacter, htmlEncodeString } from "./HTMLCharacterEntities";
...
export class StatusRegistry {
  ...
  public get registeredStatuses(): Status[] {
    return this._registeredStatuses.filter(
      ({ symbol }) => symbol !== Status.EMPTY.symbol,
    );
  }
  ...
  public getNextRecurrenceStatusOrCreate(newStatus: Status) {
    ...
    const result1 = this.getNextRecurrenceStatusOfType(
      nextStatus,
      StatusType.TODO,
    );
    if (result1) {
    ...
    const result2 = this.getNextRecurrenceStatusOfType(
      nextStatus,
  ...
  private getNextRecurrenceStatusOfType(
    nextStatus: Status,
    wanted: StatusType,
  ) {
  ...
  public findUnknownStatuses(allStatuses: Status[]): Status[] {
    ...
    unknownStatuses.forEach((s) => {
      ...
      const newStatus = StatusRegistry.copyStatusWithNewName(
        s,
        `Unknown (${s.symbol})`,
      );
    ...
    return namedUniqueStatuses.sort((status1, status2) => {
      return status1.symbol.localeCompare(status2.symbol, undefined, {
        numeric: true,
      });
    });
  }
  ...
  public mermaidDiagram(includeDetails = false) {
    ...
    uniqueStatuses.forEach((status, index) => {
      ...
      if (status.type === StatusType.DONE) {
        const nextRecurringStatus =
          this.getNextRecurrenceStatusOrCreate(status);
        const nextRecurringTypeDiffers =
          nextRecurringStatus.symbol !== nextStatus.symbol;
...
`;
  ...
  private addEdgeIfNotToInternal(
    ...
    const nextStatusIndex = uniqueStatuses.findIndex(
      (status) => status.symbol === nextStatus.symbol,
    );
  ...
  private getMermaidNodeLabel(status: Status, includeDetails: boolean) {
    ...
    if (includeDetails) {
      ...
      const statusNextStatusSymbol = htmlEncodeCharacter(
