...
export class DateParser {
  public static parseDate(input: string, forwardDate = false): Moment {
   ...
   */
  public static parseDateRange(input: string, forwardDate = false): DateRange {
  ...
  private static parseAbsoluteDateRange(
    input: string,
    forwardDate: boolean,
  ): DateRange {
  ...
  private static parseRelativeDateRange(
    input: string,
    _forwardDate: boolean,
  ): DateRange {
    const relativeDateRangeRegexp =
    ...
    if (relativeDateRangeMatch && relativeDateRangeMatch.length === 3) {
      ...
      const dateRange = DateRange.buildRelative(
        range as moment.unitOfTime.StartOf,
      );
  ...
  private static parseNumberedDateRange(
    input: string,
    _forwardDate: boolean,
  ): DateRange {
