...
import { compareByDate } from "./DateTools";
...
export class Recurrence {
    ...
    try {
      ...
      if (options !== null) {
        ...
        if (!baseOnToday && referenceDate !== null) {
          options.dtstart = moment(referenceDate)
            .startOf("day")
            .utc(true)
            .toDate();
        } else {
  ...
  public next(): {
    ...
    if (next !== null) {
      ...
      if (this.referenceDate) {
        if (this.startDate) {
          const originalDifference = moment.duration(
  ...
  private nextReferenceDateFromOriginalReferenceDate(): Moment {
    ...
    // reference date.
    const after = moment(this.referenceDate ?? undefined).endOf("day");
  ...
  private nextAfter(after: Moment, rrule: RRule): Moment {
    ...
    after.utc(true);
    let next = moment(rrule.after(after.toDate()));
  ...
  private static nextAfterMonths(
    ...
    while (
      Recurrence.isSkippingTooManyMonths(after, next, parsedSkippingMonths)
    ) {
  ...
  private static nextAfterYears(
    ...
    while (
      Recurrence.isSkippingTooManyYears(after, next, parsedSkippingYears)
    ) {
      // The next line alters `after` to be one day earlier.
      // Then returns `next` based on that.
      next = Recurrence.fromOneDayEarlier(after, rrule);
  ...
  private static fromOneDayEarlier(after: Moment, rrule: RRule): Moment {
    ...
    return moment(rrule.after(after.toDate()));
  }

  private static addTimezone(date: Moment): Moment {
    const localTimeZone = moment.utc(date).local(true);
