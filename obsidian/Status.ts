...
export class Status {
  ...
  static makeEmpty(): Status {
    return new Status(
      new StatusConfiguration("", "EMPTY", "", true, StatusType.EMPTY),
    );
  }
  ...
  static getTypeFromStatusTypeString(statusTypeAsString: string): StatusType {
    return (
      StatusType[statusTypeAsString as keyof typeof StatusType] ||
      StatusType.TODO
    );
  }
  ...
  static createFromImportedValue(imported: StatusCollectionEntry) {
    ...
    return new Status(
      new StatusConfiguration(symbol, imported[1], imported[2], false, type),
    );
  }
  ...
  public previewText() {
    let commandNotice = "";
    if (
      Status.tasksPluginCanCreateCommandsForStatuses() &&
