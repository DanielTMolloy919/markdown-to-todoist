...
import type { DefaultTaskSerializerSymbols } from "./DefaultTaskSerializer";
import { Task } from "./Task";
// import { GlobalFilter } from "../Config/GlobalFilter";
import { TaskRegularExpressions } from "./TaskRegularExpressions";
import type { SuggestInfo, SuggestionBuilder } from "./suggestorIndex";
...
function getAdjusters(dataviewMode: boolean, line: string, cursorPos: number) {
  ...
  const insertSkip =
    dataviewMode &&
    line.length > cursorPos &&
    line.charAt(cursorPos) === closingBracket
      ? 1
      : 0;
...
function addTaskPropertySuggestions(
  ...
  const hasPriority = (line: string) =>
    Object.values(symbols.prioritySymbols).some(
    ...
    if (wordUnderCursor.length >= Math.max(1, _settings.autoSuggestMinMatch)) {
      ...
      for (const filtered of filteredSuggestions) {
        const insertSkipValue =
          dataviewMode &&
          (filtered.displayText.includes("priority") ||
            filtered.displayText.includes("created"))
...
function addRecurrenceSuggestions(
  ...
  if (recurrenceMatch && recurrenceMatch.length >= 2) {
    ...
    if (recurrenceString.length > 0) {
      ...
      if (parsedRecurrence) {
        const appendedText =
          `${recurrencePrefix} ${parsedRecurrence}` + postfix;
        const insertSkipValue = dataviewMode
...
function isAnyBracketOpen(
  ...
  const numOpeningBrackets = Object.fromEntries(
    brackets.map(([open, _]) => [open, 0]),
  );
  // Maps a closing bracket to an opening bracket
  const openingOf = Object.fromEntries(
    brackets.map(([open, close]) => [close, open]),
  );
  ...
  for (const c of line) {
    if (c in numOpeningBrackets) {
      ...
      numOpeningBrackets[openingOf[c]] = Math.max(
        0,
        numOpeningBrackets[openingOf[c]] - 1,
      );
    }
  }
...
export function lastOpenBracket(
  ...
  return openBracketsStack.length > 0
    ? openBracketsStack[openBracketsStack.length - 1].bracket
    : null;
}
 ...
 */
export function onlySuggestIfBracketOpen(
  fn: SuggestionBuilder,
