...
import { PriorityTools } from "./PriorityTools";
...
import { getUserSelectedTaskFormat } from "./Settings";
...
export class Task {
  ...
  public static parseTaskSignifiers(
    ...
    // taskInfo.tags = taskInfo.tags.filter(
    //   (tag) => !GlobalFilter.getInstance().equals(tag),
    // );
  ...
  public toggle(): Task[] {
    ...
    const newStatus = StatusRegistry.getInstance().getNextStatusOrCreate(
      this.status,
    );
  ...
  public handleNewStatus(newStatus: Status): Task[] {
    ...
    const newDoneDate = this.newDate(
      newStatus,
      StatusType.DONE,
      this.doneDate,
      true,
  ...
  private createNextOccurrence(
    ...
    const cancelledDate: any = null;
    const statusRegistry = StatusRegistry.getInstance();
    const nextStatus =
      statusRegistry.getNextRecurrenceStatusOrCreate(newStatus);
    return new Task({
      ...this,
      ...nextOccurrence,
      status: nextStatus,
  ...
  public isBlocked(allTasks: Readonly<Task[]>) {
    ...
    for (const depId of this.dependsOn) {
      const depTask = allTasks.find(
        (task) => task.id === depId && !task.isDone,
      );
      if (!depTask) {
