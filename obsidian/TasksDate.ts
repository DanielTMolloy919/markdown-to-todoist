...
export class TasksDate {
  ...
  private fromNowOrder(date: moment.Moment) {
    ...
    const startDateOfThisGroup = this.fromNowStartDateOfGroup(
      date,
      earlier,
      now,
    );
    ...
    return Number(
      splitPastAndFutureDates + startDateOfThisGroup.format("YYYYMMDD"),
    );
  }
  ...
  public postpone(
    unitOfTime: unitOfTime.DurationConstructor = "days",
    amount = 1,
  ) {
